...
def seed_router_demo(db):
    utilisateur = Utilisateur(
        nom="Rakoto",
        prenom="Hery",
        email=f"hery-{uuid.uuid4().hex[:8]}@example.com",
    ...
    prefs = Preferences(profil_id=profil.id, tabous=[], allergies=[], aliments_aimes=[])
    db.add(prefs)
    db.flush()
    db.add(
        Budget(
    ...
    recette = Recette(
        nom="riz nature", heure_conseillee=time(12, 0), kcal_total=400, tags=["dejeuner"]
    )
    db.add(recette)
    db.flush()
    db.add(
    ...
    db.commit()
    return utilisateur, profil, riz, planning, repas


def auth_headers(utilisateur: Utilisateur) -> dict[str, str]:
    return {"X-API-Token": utilisateur.api_token}
